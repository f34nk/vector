...
fn rename_referrer(mut input: Value) -> Value {
    if let Some(obj) = input.as_object_mut() {
        if let Some(referrer) = obj.remove("referrer") {
            obj.insert("referer".into(), referrer);
        }
    }
    input
}
...
impl Expression for ParseNginxLogFn {
    fn resolve(&self, ctx: &mut Context) -> Resolved {
        ...
        log_util::log_fields(&regex, &captures, &timestamp_format)
            .map(rename_referrer)
            .map_err(Into::into)
    }
